...
### Адресные E2E без постоянного отвлечения

Исходный ввод:

```text
уменьшить количество E2E тестов, которые на реальном приложении
тестировать только затронутые вещи
проверь, что уведомления через какое-то время сами закрываются
```

Нормализация:

- В интерактивной работе не запускать весь набор реальных окон после каждой правки.
- Разделить app E2E по функциональным областям и позволить запускать одну или несколько областей.
- Полный видимый прогон оставить для широких изменений и релизной проверки.
- Проверять auto-dismiss уведомления до фактического удаления toast view после анимации.

Статус:

- Выполнено: добавлены адресные app-сценарии, одна test-сборка переиспользуется в рамках запуска,
  а notification E2E подтверждает timeout и удаление view.
