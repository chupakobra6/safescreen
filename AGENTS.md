...
- Разработка полностью агентная: не планировать workflow, требующий ручной работы в Xcode GUI.
  Baseline для будущего кода - CLI/SwiftPM через Command Line Tools; Xcode GUI только optional для
  profiling/signing, если позже станет нужен.
