...
- автоматическое скрытие окна при клике вне `BrowserPanel`;
...
Клик вне окна скрывает панель и выводит ее из input mode. Для этого используется global mouse-down
monitor AppKit; `sharingType = .none` при этом не меняется.
